...
def clean_rename_with_ai():
    ...
    for md_file in out_dir.rglob("*.md"):
        ...
        # 이미 이름이 바뀐 파일('post_' 또는 '제목없음'으로 시작하지 않는 파일)은 건너뜀
        if not md_file.name.startswith("post_") and not md_file.name.startswith("제목없음"): 
            continue
    ...
    for i, md_file in enumerate(md_files, 1):
        ...
        for attempt in range(3):
            try:
                ...
                raw_title = response_text.replace('"', '').replace("'", "")
                ...
                if "429" in str(e):
                    print(f"  [!] 요청 한도 초과(429), 10초 대기 후 재시도... (시도 {attempt+1}/3)")
                    time.sleep(10)
                    continue
                print(f"  [!] LLM 호출 오류: {e}")
                break
