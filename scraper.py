...
def _scrape_single_post(page: Page, url: str, index: int) -> dict[str, Any] | None:
    ...
    try:
        ...
        time.sleep(3)
        if "/post/" not in page.url:
            print(f"  [!] 게시물이 삭제되었거나 리다이렉트됨 ({url} -> {page.url})")
            return None
    except Exception as e:
    ...
    try:
        ...
        # 첫 번째 라인의 작성자명 및 시간 접두어(예: 'ye____i_n 22시간 ...') 정리
        while texts and post_data["author"]:
            first = texts[0].strip()
            if first.lower().startswith(post_data["author"].lower()):
                first = first[len(post_data["author"]):].strip()
            first = re.sub(r'^(?:[^\n]{1,15}\s+)?\d+(?:시간|[일분초])\s*', '', first).strip()
            if first in ['간', '시', '일', '초', '분', '·']:
                texts.pop(0)
                continue
            if first:
                texts[0] = first
        ...
        # ── 댓글 추출 (관련 스레드 / 추천 글 제외) ──
        # Threads는 댓글 하단에 '관련 스레드(Related threads)' 추천 피드를 무한으로 붙입니다.
        # 따라서 구분선(Divider) 이전의 실제 답글 컨테이너만 필터링하여 추출합니다.
        try:
            extracted_comments = page.evaluate("""() => {
                ...
                // '관련 스레드' / 'Related threads' 구분선 찾기
                const allSpansAndDivs = Array.from(document.querySelectorAll('span, h1, h2, h3, h4, div'));
                const divider = allSpansAndDivs.find(el => {
                    if (el.children.length > 1) return false;
                    const t = el.innerText?.trim();
                    return t && /^(Related threads|관련 스레드|추천 스레드|스레드 더 보기|More threads|Suggested threads|관련 게시물|추천 게시물|더 많은 스레드|회원님을 위한 추천)$/i.test(t);
                });

                const results = [];
                // 첫 번째(0)는 원본 글이므로 1번부터 검사
                for (let i = 1; i < containers.length; i++) {
                    const c = containers[i];

                    // 구분선이 존재하고, 해당 컨테이너가 구분선 뒤에 위치하면 추천 피드이므로 중단
                    if (divider) {
                        const pos = divider.compareDocumentPosition(c);
                        if (pos & Node.DOCUMENT_POSITION_FOLLOWING) {
                            break;
                        }
                    }

                    const authorEl = c.querySelector('a[href^="/@"] span');
            ...
            for c_data in extracted_comments:
                post_data["comments"].append(c_data)
        except Exception as ce:
            print(f"  [!] 댓글 추출 중 오류: {ce}")
...
def scrape_saved_posts() -> list[dict[str, Any]]:
    ...
    with sync_playwright() as pw:
        ...
        # 개별 글 스크래핑을 위해 비인증 클린 페이지 생성
        # (Threads는 로그인 세션으로 개별 글 URL에 직접 접속 시 홈 피드로 리다이렉트하지만, 비인증 상태에서는 정상적으로 개별 글 페이지와 댓글을 제공합니다)
        anon_context = browser.new_context(
            viewport={"width": 1280, "height": 900},
            locale="ko-KR",
        )
        post_page = anon_context.new_page()

        # 개별 글 스크래핑
        print(f"\n[*] {len(new_links)}개 글(새로운 글) 상세 스크래핑 시작...")
        for i, link in enumerate(new_links, 1):
            print(f"\n[{i}/{len(new_links)}] {link}")
            post = _scrape_single_post(post_page, link, i)
            if post:
                posts.append(post)
